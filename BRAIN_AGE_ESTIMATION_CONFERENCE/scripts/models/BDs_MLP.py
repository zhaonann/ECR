...
class Diag_MLP(nn.Module):
    def __init__(self, in_channels=1, n_classes=3):
        '''
        in_channels: 1
        '''
        super(Diag_MLP, self).__init__()
        ...
        self.diag_blk = nn.Sequential(
            nn.ReLU(),
            nn.Linear(feats[3]+4, feats[3]),
            nn.ReLU(),
            nn.BatchNorm1d(num_features=feats[3]),
            nn.Dropout(0.5),
            nn.Linear(feats[3], feats[3]),
            nn.ReLU(),
            nn.BatchNorm1d(num_features=feats[3]),
            nn.Dropout(0.5),
            nn.Linear(feats[3], n_classes)
        )
        

    def forward(self, x_3d, x1, x2, x3, GAF, age_gt, sex):
        ...
        out = torch.cat((out0, out1, out2, out3, out4), dim=1)
        out = self.fusion_blk(out)
        age_pred = self.age_blk(out)

        BAG = age_pred - age_gt
        in_diag = torch.cat((out, BAG, age_gt, age_pred, sex), dim=1) 
        cog = self.diag_blk(in_diag)
