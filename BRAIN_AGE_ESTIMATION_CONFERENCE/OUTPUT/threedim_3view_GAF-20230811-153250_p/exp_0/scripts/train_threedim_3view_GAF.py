...
def main():
    ...
    args.note = ""
    args.parsave = "{}-{}".format('threedim_3view_GAF', time.strftime("%Y%m%d-%H%M%S")) # spectral graph conv
    ...
    for exp in range(0, args.n_exps):
        ...
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "models", "threedim_3view_GAF_MSA_share_encoder.py")))
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "train_threedim_3view_GAF.py")))
        ...
        dataset = ExCustomDataset_threedim_3view_GAF(df=df, transforms=True)
        ...
        if args.pretrain_3D:
            ...
            threedim_path = "/public/bme/home/zhaonan/brain_age_est/codes/BRAIN_AGE_ESTIMATION_CONFERENCE/OUTPUT/\
                            threedim-20230808-215808/exp_0/WEIGHTS/model_s_best_weight.pkl"
            utils.load_pretrain_3d(model = model_s, model_path_3d = threedim_path)
            print("Load pretrain 3d model!", flush=True)
        ...
        for epoch in range(1, args.n_epochs+1):
            ...
            for data in train_queue:
                sMRI, axial, sagittal, coronal, GAF, labels = data[:6]
                sMRI = sMRI.to(device)
        ...
        in_test_mae_s = preds_store(model_s, in_test_queue, n_in_tests, stage="last_in_test", device=device, path_figs=path_figs, exp=exp)
        ...
        in_test_mae_s = preds_store(model_s, in_test_queue, n_in_tests, stage="best_in_test", device=device, path_figs=path_figs, exp=exp)
