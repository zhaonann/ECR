...
def load_pretrain_unet(model, model_path_unet3D):
    state_dict = model.state_dict()
    pre_s_dict = torch.load(model_path_unet3D)
    pre_s_new = {}
...
def load_pretrain_3d_2d(model, model_path_s1, model_path_s2):
    """
    Load the pre-trained 3d and 2d model weights
    """
    state_dict = model.state_dict()

    pre_s_dict = torch.load(model_path_s1)
    ...
    pre_s_dict = torch.load(model_path_s2)
