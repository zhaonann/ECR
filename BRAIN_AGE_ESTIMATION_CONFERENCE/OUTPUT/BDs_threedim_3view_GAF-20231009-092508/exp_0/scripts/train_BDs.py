...
import utils
from data import ExCustomDataset_BDs_MLP, data_split_diag, train_val_test_split, train_val_test_split_stratify
...
def main():
    ...
    for exp in range(0, args.n_exps):
        ...
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "train_BDs.py")))
        ...
        df_train, df_val, df_in_test, _, n_trains, n_vals, n_in_tests, _ = train_val_test_split_stratify(data_csv)
        ...
        for epoch in range(1, args.n_epochs+1):
            ...
            logging.info("******* Epoch %d, Train CE Loss %.2f, Val CE Loss %.2f, Train ACC %.2f, Val ACC %.2f *******", epoch, train_loss, loss_dict['sMRI'], train_acc, acc_dict['sMRI'])
        ...
        val_acc_s = preds_store(model_s, val_queue, n_vals, stage="last_val", device=device, path_figs=path_figs, exp=exp, target_name=target_name, n_classes=n_classes)
        ...
        logging.info("*******Again Last Epoch %d,  Val ACC %.2f, In Test ACC %.2f *******",
                     args.n_epochs, val_acc_s, in_test_acc_s)
        ...
        logging.info("******* Best Epoch %d, Train ACC %.2f, Val MAE %.2f, In Test MAE %.2f *******", 
                     val_opt["Epoch"], val_opt["Opt_train"], val_opt["Opt_val"], in_test_acc_s)
      
        data_ACC[exp][:2] = val_opt["Opt_val"], in_test_acc_s
