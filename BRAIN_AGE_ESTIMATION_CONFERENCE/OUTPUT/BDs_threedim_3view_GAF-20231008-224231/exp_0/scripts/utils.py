...
def init():
    ...
    parser.add_argument("--pretrain_age_model", type=bool, default=True, help="Input if you need a pre-trained age estimation model")
    parser.add_argument("--diag_mode", type=str, default="CN_MCI_AD", help="mode of diagnosis")
...
def load_pretrain_age_model(model, model_path_pretrain):
    """
    Load the pre-trained age model
    """
    state_dict = model.state_dict()

    pre_s_dict = torch.load(model_path_pretrain)

    pre_s_new = {}

    for k, v in pre_s_dict.items():
        pre_s_new[k] = pre_s_dict[k] 
